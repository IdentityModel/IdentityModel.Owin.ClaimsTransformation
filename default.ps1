properties {
	...
	$sln_file = "$src_directory\Owin.ClaimsTransformation.sln"
...
task CreateNuGetPackage -depends Compile {
	...
	copy-item $src_directory\Owin.ClaimsTransformation.nuspec $dist_directory

	new-item $dist_directory\lib\net45 -type directory
	copy-item $output_directory\IdentityModel.Owin.ClaimsTransformation.dll $dist_directory\lib\net45\
	copy-item $output_directory\IdentityModel.Owin.ClaimsTransformation.pdb $dist_directory\lib\net45\
	copy-item $output_directory\IdentityModel.Owin.ClaimsTransformation.xml $dist_directory\lib\net45\

	exec { . $nuget_path pack $dist_directory\Owin.ClaimsTransformation.nuspec -BasePath $dist_directory -o $dist_directory -version $packageVersion }
}
